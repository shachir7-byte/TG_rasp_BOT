...
# 📝 ЛОГИРОВАНИЕ (ТОЛЬКО ФАЙЛ + КРИТИЧЕСКИЕ ОШИБКИ В КОНСОЛЬ)
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

# Файловый обработчик (ВСЁ: DEBUG, INFO, WARNING, ERROR)
...
# Консольный обработчик (ТОЛЬКО CRITICAL и ERROR)
...
# --- БАЗА ДАННЫХ (ЕДИНОЕ СОЕДИНЕНИЕ) ---
async def init_db():
    global db_connection
    db_connection = await aiosqlite.connect(DB_FILE)
    
    await db_connection.execute("""CREATE TABLE IF NOT EXISTS users (
    ...
    await db_connection.execute("""CREATE TABLE IF NOT EXISTS stats (
        id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id INTEGER, action TEXT, timestamp TEXT)""")
    
    await db_connection.execute("""CREATE TABLE IF NOT EXISTS groups_config (
    ...
    await db_connection.executemany("INSERT OR IGNORE INTO groups_config (key, name, group_id) VALUES (?, ?, ?)", default_groups)
    await db_connection.commit()
    logger.info("База данных инициализирована (Единое соединение)")

# Хелперы БД (используют глобальное соединение)
async def save_user(chat_id: int, username: str = None, group: str = 'is'):
    now = datetime.now(LOCAL_TIMEZONE).isoformat()
    await db_connection.execute("INSERT OR IGNORE INTO users (chat_id, username, user_group, first_seen, request_count) VALUES (?, ?, ?, ?, 0)", (chat_id, username, group, now))
    await db_connection.execute("UPDATE users SET username = COALESCE(?, username), user_group = COALESCE(?, user_group) WHERE chat_id = ?", (username, group, chat_id))
    await db_connection.commit()

async def get_user_group(chat_id: int) -> str:
    async with db_connection.execute("SELECT user_group FROM users WHERE chat_id = ?", (chat_id,)) as cursor:
        row = await cursor.fetchone()
        return row[0] if row else 'is'

async def increment_request_count(chat_id: int):
    await db_connection.execute("UPDATE users SET request_count = request_count + 1 WHERE chat_id = ?", (chat_id,))
    await db_connection.commit()

async def log_action(chat_id: int, action: str):
    await db_connection.execute("INSERT INTO stats (chat_id, action, timestamp) VALUES (?, ?, ?)", (chat_id, action, datetime.now(LOCAL_TIMEZONE).isoformat()))
    await db_connection.commit()

async def get_user_notify(chat_id: int):
    async with db_connection.execute("SELECT notify_time, notify_type, notify_offset FROM users WHERE chat_id = ?", (chat_id,)) as cursor:
        row = await cursor.fetchone()
        return row if row else (None, 'time', 0)

async def save_notify(chat_id: int, time: str = None, notify_type: str = 'time', offset: int = 0):
    await db_connection.execute("UPDATE users SET notify_time = ?, notify_type = ?, notify_offset = ? WHERE chat_id = ?", (time, notify_type, offset, chat_id))
    await db_connection.commit()

async def cancel_notify(chat_id: int):
    await db_connection.execute("UPDATE users SET notify_time = NULL, notify_type = 'time', notify_offset = 0 WHERE chat_id = ?", (chat_id,))
    await db_connection.commit()

async def get_active_users():
    async with db_connection.execute("SELECT chat_id, notify_time, notify_type, notify_offset, last_sent_date FROM users WHERE notify_time IS NOT NULL OR (notify_type = 'offset' AND notify_offset > 0)") as cursor:
        return await cursor.fetchall()

async def update_last_sent(chat_id: int, date_str: str):
    await db_connection.execute("UPDATE users SET last_sent_date = ? WHERE chat_id = ?", (date_str, chat_id))
    await db_connection.commit()

async def get_all_users_chat_ids():
    async with db_connection.execute("SELECT chat_id FROM users") as cursor:
        rows = await cursor.fetchall()
        return [r[0] for r in rows]

async def get_user_count():
    async with db_connection.execute("SELECT COUNT(*) FROM users") as cursor:
        row = await cursor.fetchone()
        return row[0]

async def get_groups_config():
    async with db_connection.execute("SELECT key, name, group_id FROM groups_config") as cursor:
        rows = await cursor.fetchall()
...
async def get_full_admin_stats():
    ...
    for chat_id, username, grp, count, first_seen, last_act in users:
        ...
        # Используем <code> для ID и количества для удобного копирования
        lines.append(f"🆔 <code>{chat_id}</code> | <b>{name}</b> ({grp_name})")
        lines.append(f"   └ Запросов: <code>{count}</code> | Активность: {act_info}\n")
    
    return "\n".join(lines)

async def clear_stats():
    await db_connection.execute("DELETE FROM stats")
    deleted = db_connection.total_changes
    await db_connection.commit()
    return deleted
    ...
    if is_week:
        ...
        for idx, date_str in enumerate(sorted_dates):
            ...
            for l in day_lessons:
                ...
                lines.append(f"<b><code>{l['time']}</code></b>")
...
# 🔘 КЛАВИАТУРЫ (ДИНАМИЧЕСКИЕ И СТИЛИ)
async def get_group_select_keyboard():
    groups = await get_groups_config()
    buttons = []
    for key, info in groups.items():
        buttons.append([InlineKeyboardButton(text=f"📚 {info['name']}", callback_data=f"group_{key}")])
...
def get_notify_main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        # Красная кнопка для удаления (danger)
        [InlineKeyboardButton(text="🗑 Отключить", callback_data="notify_delete", callback_data_payload="danger")], 
...
async def cmd_start(m: types.Message, state: FSMContext):
    ...
    kb = await get_group_select_keyboard()
    await m.answer("👋 <b>Привет! Я бот-расписание</b>\n\nВыберите группу:", reply_markup=kb, parse_mode="HTML")
...
async def handle_change_group(c: types.CallbackQuery, state: FSMContext):
    ...
    try:
        kb = await get_group_select_keyboard()
        await c.message.edit_text("🔄 <b>Сменить группу</b>", reply_markup=kb, parse_mode="HTML")
    except TelegramBadRequest: pass
    await c.answer()

# 🔐 АДМИН ПАНЕЛЬ
@dp.callback_query(lambda c: c.data == "admin_panel")
async def handle_admin_panel(c: types.CallbackQuery, state: FSMContext):
...
async def handle_admin_clear(c: types.CallbackQuery):
    ...
    try:
        await c.message.edit_text(f"🗑 <b>Логи очищены!</b>\nУдалено: <code>{deleted}</code>", parse_mode="HTML", reply_markup=get_admin_keyboard())
    except TelegramBadRequest: pass
    await c.answer()
...
async def notify_time_custom(c: types.CallbackQuery, state: FSMContext):
    await state.set_state(UserState.waiting_time_input)
    try:
        await c.message.edit_text(
            "✏️ <b>Ввод времени</b>\n\n"
...
# ОБРАБОТЧИК ТЕКСТА С FSM
@dp.message(StateFilter(UserState.waiting_time_input))
async def process_time_input(m: types.Message, state: FSMContext):
    text = m.text.strip()
    if re.match(r"^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$", text):
        await save_notify(m.chat.id, text, 'time', 0)
        await log_action(m.chat.id, f"notify_custom_{text}")
        await state.clear()
        await m.answer(f"✅ Установлено на <b><code>{text}</code></b>", parse_mode="HTML", reply_markup=get_notify_main_menu())
    else:
        await m.answer("❌ Неверный формат. Пример: <code>14:30</code>\nПопробуй еще раз или /cancel", parse_mode="HTML")

@dp.message(StateFilter(AdminState.waiting_broadcast))
async def process_broadcast(m: types.Message, state: FSMContext):
    ...
    if target_id:
        try:
            ...
            await m.answer(f"✅ Отправлено пользователю <code>{target_id}</code>", parse_mode="HTML")
            await log_action(m.chat.id, f"broadcast_to_{target_id}")
        except Exception as e:
            await m.answer(f"❌ Ошибка: {e}")
    else:
        users = await get_all_users_chat_ids()
        success, failed = 0, 0
        await m.answer(f"📢 Рассылка для <code>{len(users)}</code>...")
        for uid in users:
            try:
        ...
        await m.answer(f"✅ Успешно: <code>{success}</code>\n❌ Ошибок: <code>{failed}</code>", parse_mode="HTML")
        await log_action(m.chat.id, f"broadcast_all_{success}_{failed}")
    
    await state.clear()

# Обычные сообщения (если не в состоянии)
@dp.message(lambda m: m.text and not m.text.startswith('/'))
async def handle_unknown_text(m: types.Message):
    await m.answer(
...
async def notify_menu(c: types.CallbackQuery):
    ...
    status = "🔕 Отключено"
    if n_time: status = f"🕐 <code>{n_time}</code>"
    elif n_type == 'offset' and n_off > 0: status = f"⏳ За <code>{n_off}</code> ч"
    try:
...
async def notify_set_time(c: types.CallbackQuery, state: FSMContext):
    val = c.data.replace("notify_time_", "")
    if val == "custom":
        return # Обработано в notify_time_custom
    await state.clear()
    await save_notify(c.from_user.id, val, 'time', 0)
    await log_action(c.from_user.id, f"notify_{val}")
    try:
        await c.message.edit_text(f"✅ Установлено на <code>{val}</code>", parse_mode="HTML", reply_markup=get_notify_main_menu())
    except TelegramBadRequest: pass
    await c.answer()

@dp.callback_query(lambda c: c.data.startswith("notify_offset_"))
async def notify_set_offset(c: types.CallbackQuery, state: FSMContext):
    await state.clear()
    ...
    try:
        await c.message.edit_text(f"✅ Установлено: за <code>{off}</code> ч", parse_mode="HTML", reply_markup=get_notify_main_menu())
    except TelegramBadRequest: pass
    await c.answer()

@dp.callback_query(lambda c: c.data == "notify_delete")
async def notify_del(c: types.CallbackQuery, state: FSMContext):
    await state.clear()
...
# 🚀 ЗАПУСК
async def run_bot():
    global GLOBAL_SESSION, db_connection
